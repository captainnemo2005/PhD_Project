...
public class Controller {
	    ...
	    public Controller(double lambda, double mu, double endTime){
	    	this.lambda = lambda;
	    	this.mu     = mu;
	    ...
	    public void scheduleDepart(double arrivalTime) {
	    	this.nextDeparture = this.clock + GaussianDistribs(mu); // schedule the time as GaussianDis for nextDepart
	    ...
	    public void run(PrintWriter out) {
	    	while(this.clock < this.endTime) {
	    		if(checkpoints.get(0) < nextArrival && checkpoints.get(0) < nextDeparture) {
	    			...
	    			this.requests++;
	    		}else { // element has been serviced, remove it from the queue
	    ...
	    public void printStats(PrintWriter out) {
	    	...
	    	out.println("Average waiting time = " + Tw/requests);
	    	out.println("Average time in the queue = " + Tq/serviced);
	    	out.println("Average service time = " + Ts/serviced);

	    	System.out.println("\nSTATISTICS OF RUN");
	    	System.out.println("Total number of waiting customers = " + w);
	    	System.out.println("Total number of customer in the queue = " + q);
	    	System.out.println("Average waiting time = " + Tw/requests);
	    	System.out.println("Average time in the queue = " + Tq/serviced);
	    	System.out.println("Average service time = " + Ts/serviced);
	    }
